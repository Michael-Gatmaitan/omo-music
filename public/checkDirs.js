...
console.log(fs.readdirSync("music-data").length, bodyData.length);

const fixFileNames = () => {
  bodyData.forEach((data, i) => {
    const { path, musics } = data;

    const folderMusics = fs.readdirSync(path);

    if (folderMusics.length !== musics.length) {
      // console.log(`Path music length is not equal to bodydata: ${path}`);
      return;
    }

    folderMusics.forEach((music, j) => {
      const bodyDataMusic = bodyData[i].musics[j];

      // check if same

      if (music !== bodyDataMusic && music.includes("_")) {
        const newFileName = `${path}/${bodyDataMusic}`;
        const bodyDataMusicExists = fs.existsSync(newFileName);
        // const
        const currentFileName = `${path}/${music}`;
        const isMusicExists = fs.existsSync(currentFileName);

        if (!bodyDataMusicExists && isMusicExists) {
...
// fixFileNames();
// Zild - A Love Song.mp3
// fs.rename("zild/Zild - A Love Song.mp3", "zild/pakyu.mp3", (err) => {
//   if (err) throw err;
...
// console.log(`${artistsNotExists.length} folders do not exists`);
